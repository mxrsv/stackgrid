...
43. **One notification per transition**: each attention transition sends at most one native notification, guaranteed by two complementary layers — the tab-manager dedupes on the attention latch identity (last forwarded kind per pane, so only a newly raised or escalated kind is forwarded), and the notifier dedupes on revision as a harmless second layer; a phase-only re-emit of an already-latched kind, or any later re-render or poll of the same transition, never sends a duplicate.
