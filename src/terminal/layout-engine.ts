...
export function createLayoutEngine(container: HTMLElement, host: LayoutEngineHost): LayoutEngine {
    ...
    function applyOverlay(input: LayoutOverlayInput): void {
        applyRatios(container, computeDisplayTree(input.tree, input.activeId, input.focusExpand, input.paneCount));
    }
...
function buildDivider(node: SplitNode, path: Path, elements: SplitElements, ctx: LayoutContext): HTMLElement {
